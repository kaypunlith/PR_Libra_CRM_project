...
public class OrganizationServiceImpl implements OrganizationService {
    ...
    public ResponseMessage<BaseResult> insert(OrganizationRequest request, BindingResult bindingResult, HttpServletRequest httpServletRequest) throws UnknownHostException {
        ...
        try {
            // Check Permission
            User authUser = userService.getUserAuth();
            System.out.println("authUser "+authUser);
            if (authUser == null || authUser.getId() == null) {
                return ResponseMessageUtils.makeResponse(false, 401, "unauthorized", "User not found or invalid session");
            }
            ...
            if (result) {
                ...
                if (request.getCloneId() != null){
                    ...
                    helperMapper.updateCode("customers", "customer_code", generateCode.generateNextCustomerCode("LPSA","customer_code","customers",true), organization.getId());
                } else {
                    // Add
                    System.out.println("Function generate code");
                    String code = generateCode.generateNextCustomerCode("LPSA","customer_code","customers",true);
