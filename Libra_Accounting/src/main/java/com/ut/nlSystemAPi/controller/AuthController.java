...
//    private final GoogleAuthenticator gAuth;
    ...
    static {
        ...
        PROJECT_NAME = "logs/update-share/nlSystemAccAPi";
...
//    public ResponseMessage<BaseResult> generate(@PathVariable String username, HttpServletResponse response) {
////        final GoogleAuthenticatorKey key = gAuth.createCredentials(username);
//
////    QRCodeWriter qrCodeWriter = new QRCodeWriter();
////
////    String otpAuthURL = GoogleAuthenticatorQRGenerator.getOtpAuthTotpURL("JTI Fleet", username, key);
////
////    BitMatrix bitMatrix = qrCodeWriter.encode(otpAuthURL, BarcodeFormat.QR_CODE, 200, 200);
//
//        final String uuid = UUID.randomUUID().toString();
//        String filePath = ROOT + File.separator + PROJECT_NAME + File.separator + FOLDER_UPLOAD + "/"+ uuid + ".JPG";
...
//        authMapper.updateUser("/"+ FOLDER_UPLOAD + "/"+ uuid + ".JPG", username, secretKey);
//        return ResponseMessageUtils.makeResponse(true, messageService.message("Success", Collections.singletonList(FOLDER_UPLOAD + "/"+ uuid + ".JPG"), true));
//    }
    ...
    public ResponseMessage<BaseResult> generate(@PathVariable String username, HttpServletResponse response) {
        ...
        final String contextPath = "logs/update-share" + environment.getProperty("server.servlet.context-path");
        ...
        // Convert BitMatrix to byte array (JPG format)
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        MatrixToImageWriter.writeToStream(bitMatrix, "JPG", baos);
        byte[] qrCodeBytes = baos.toByteArray();
        DiskFileItem fileItem = new DiskFileItem("qrCode", "image/jpeg", false,
                UUID.randomUUID().toString() + ".JPG", qrCodeBytes.length, null);
        fileItem.getOutputStream().write(qrCodeBytes);
        MultipartFile qrCodeFile = new CommonsMultipartFile(fileItem);
        String fileSignature = FileUploadUtils.saveFileUploaded(qrCodeFile, contextPath);
    ...
    //    outputStream.close();
        ...
        String[] parts = fileSignature.split("/", 2);
        // return ResponseMessageUtils.makeResponse(true, messageService.message("Success", Collections.singletonList(FOLDER_UPLOAD + "/"+ uuid + ".JPG"), true));
...
//                authMapper.updateStatusLogin(body.getUsername());
                ...
                return ResponseMessageUtils.makeResponse(true, messageService.message("Code Invalid", false));
            }
        }else {
